				...
				<div class="col-xl-6">
					<div class="panel panel-inverse">
						...
						<div class="panel-body">
							<form action="?c=user&m=doUpdate" method="POST">
								<input type="hidden" id="ip-uuid" name="ip-uuid" value="<?= $user['uuid']; ?>">
								<div class="form-floating">
								  	<input name="ip-username" id="ip-username" class="form-control fs-15px"  value="<?= $user['username']; ?>" readonly disabled/>
								...
								<div class="form-floating mt-1">
								  	<input name="ip-full-name" id="ip-full-name" class="form-control fs-15px" value="<?= $user['full_name']; ?>"/>
								  	<label for="ip-full-name" class="d-flex align-items-center fs-13px">Tên</label>
								</div>

								<div class="form-floating mt-1">
								  	<input name="ip-phone" id="ip-phone" class="form-control fs-15px" value="<?= $user['phone']; ?>"/>
								  	<label for="ip-phone" class="d-flex align-items-center fs-13px">Số ĐT</label>
								</div>

								<div class="form-floating mt-1">
								  	<input name="ip-address" id="ip-address" class="form-control fs-15px" value="<?= $user['address']; ?>"/>
								  	<label for="ip-address" class="d-flex align-items-center fs-13px">Địa chỉ</label>
								</div>

								<div class="form-floating mt-1">
								  	<input name="ip-email" id="ip-email" class="form-control fs-15px" value="<?= $user['email']; ?>"/>
								  	<label for="ip-email" class="d-flex align-items-center fs-13px">Mail</label>
								</div>

								<div class="form-floating mt-1">
								  	<input name="ip-birth" id="ip-birth" class="form-control fs-15px" type="date" value="<?= $user['birth']; ?>"/>
								...
								<div class="form-floating mt-1">
									<select name="ip-role" id="ip-role" class="form-control fs-15px">
										<?php foreach ($role as $key => $item) : ?>
											<option <?= ($user['id_role'] == $item['id']) ? "selected" : ""; ?> value="<?= $item['id']; ?>"><?= $item['name']; ?></option>
										<?php endforeach; ?>
									</select>
