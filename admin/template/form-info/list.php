		...
		<div id="content" class="app-content">
			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					<form method="GET">
						<input type="text" name="c" class="d-none" value="formInfo">
						...
						<div class="row">
							<div class="col-xxl-2 col-xl-3 col-md-4">
								<div class="input-group mb-3">
									...
									<select name="ip-form-group" id="ip-form-group" class="form-control">
										<option value="">Tất cả</option>
										<?php foreach ($formGroups as $key => $item) : ?>
											<option <?= ($search['idFormGroup'] == $item['id']) ? "selected" : ""; ?> value="<?= $item['id']; ?>"><?= $item['name']; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
							</div>
							<div class="col-xxl-2 col-xl-3 col-md-4">
								<div class="input-group mb-3">
									...
									<input name="ip-code" id="ip-code" class="form-control" value="<?= $search['code']; ?>"/>
								</div>
							</div>
							<div class="col-xxl-2 col-xl-3 col-md-4">
								<div class="input-group mb-3">
			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							<?php if(count($formInfos) > 0) : ?>
                      		<?php foreach ($formInfos as $key => $item) : ?>
                      		<tr>
                      			<td><?= $key + 1; ?></td>
                      			<td><?= $item['form_group_name']; ?></td>
                      			<td><img src="../<?= $item['thumb_url']; ?>" class="rounded h-20px"></td>
                      			<td><label class="mw-80px text-truncate" title="<?= $item['code']; ?>"><?= $item['code']; ?></label></td>
                      			<td><?= $item['name']; ?></td>
                      			<td><?= $item['description']; ?></td>
                      			<td><?= $item['created_date']; ?></td>
                      			<td><?= $item['updated_date']; ?></td>
                      			<td><a href="?c=formInfo&m=goUpdate&code=<?= $item['code']; ?>"><i class="fa fa-cog"></i></a></td>
                      		</tr>
                      		<?php endforeach; ?>
                      		<?php else : ?>
                      			<tr>
